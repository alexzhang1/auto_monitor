...
def ping_monitor_task():
    ...
    pddata = pd.read_csv('./config/cust_server_list_test_null.csv',encoding='gbk')
    ...
    try:
        ...
        for info in sort_df.iterrows():                      
            ping_info_verify = ping_server_info(info)
            ping_Check_flag = ping_info_verify
            if ping_Check_flag:
