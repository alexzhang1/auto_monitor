            ...
            for filespath in files:
                ...
                if os.path.isfile(local_file):
                    ...
                    try:
                        ...
                        logger.info("ee,Upload %s to remote %s" % (local_file, remote_file))
                        sftp.put(local_file, remote_file)
                        suffix = remote_file.split('.')[-1]
                        if suffix == 'sh':
...
#        print "all_files:", all_files
        ...
        for x in all_files:            
            ...
            logger.info(u'文件 %s 传输中...' % filename)
...
#                    remote_dir = '/home/trade/temp'
                ...
                if lindir !='':
                    remote_dir = lindir 
                t = MyThread(upload, (local_upload, remote_dir, hostip, port, username, password, singlefile,), upload.__name__ + str(i))
                threads.append(t)
...
#                threads=[]
            for (i,info) in zip(thrlist, linuxInfo):
                ...
                if lindir !='':
                    remote_dir = lindir
                t = MyThread(download, (local_download, remote_dir, hostip, port, username, password, singlefile,), download.__name__ + str(i))
                threads.append(t)
        else:
